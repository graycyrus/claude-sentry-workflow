...
REPO_URL="https://github.com/graycyrus/claude-sentry-workflow.git"
SKILLS_LIST="brb-sentry-workflow brb-sentry-triage brb-sentry-analyze brb-sentry-raise-issue brb-sentry-update"
...
if [ -t 0 ]; then
  echo "claude-sentry-workflow installer"
  echo "================================"
  echo ""
  echo "Install method:"
  echo ""
  echo "  1) Copy — Global   (~/.claude/skills/)     — simple, update via /brb-sentry-update"
  echo "  2) Copy — Project  (.claude/skills/)        — per-project, update via /brb-sentry-update"
  echo "  3) Symlink — Global (~/.claude/skills/)     — git pull = instant updates"
  echo ""
  read -rp "Choose [1/2/3]: " choice
else
  choice="1"
  echo "claude-sentry-workflow installer (non-interactive — installing globally via copy)"
fi
...
if [ "$METHOD" = "symlink" ]; then
  CLONE_DIR="$HOME/.claude/claude-sentry-workflow"
  ...
  if [ -d "$CLONE_DIR" ]; then
    ...
    echo ""
    echo "Cloning claude-sentry-workflow to $CLONE_DIR..."
  ...
  echo ""
  echo "Cloning claude-sentry-workflow..."
  ...
  for skill in "$TARGET"/brb-sentry-*/; do
    name=$(basename "$skill")
    echo "  /$name"
  done
